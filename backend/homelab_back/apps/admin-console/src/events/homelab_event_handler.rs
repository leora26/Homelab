...
use derive_new::new;
use homelab_core::events::{FileUpdatedEvent, FileUploadedEvent, UserCreatedEvent, UserUpdatedEvent, WhiteListedUserCreatedEvent, WhiteListedUserUpdatedEvent};
...
impl EventHandler for HomelabEventHandler {
    async fn handle(&self, routing_key: &str, data: &[u8]) -> Result<(), String> {
        match routing_key {
            ...
            "file.uploaded" => {
                let event: FileUploadedEvent = serde_json::from_slice(data)
                    .map_err(|e| format!("Json Error: {}", e))?;

                println!("👤 Handling File Upload: {}", event.file_id);

                self.file_service.log_new_file(event).await
                    .map_err(|e| format!("Error handling updated WhiteListedUser: {}", e))?;

                Ok(())
            },
            "file.updated" => {
                let event: FileUpdatedEvent = serde_json::from_slice(data)
                    .map_err(|e| format!("Json Error: {}", e))?;

                println!("👤 Handling File Upload: {}", event.file_id);

                self.file_service.log_updated_file(event).await
                    .map_err(|e| format!("Error handling updated WhiteListedUser: {}", e))?;
