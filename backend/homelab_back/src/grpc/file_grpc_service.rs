...
impl FileService for GrpcFileService {
    async fn get_file(&self, request: Request<GetFileRequest>) -> Result<Response<FileResponse>, Status> {
        ...
        let file = self.app_state.file_service.get_by_id(file_id)
            .await?
    ...
    async fn search_files(&self, request: Request<SearchFilesRequest>) -> Result<Response<FileListResponse>, Status> {
        let req = request.into_inner();

        let files = self.app_state.file_service.search_file(req.file_name).await?;
    ...
    async fn get_deleted_files(&self, _request: Request<()>) -> Result<Response<FileListResponse>, Status> {
    ...
    async fn init_file(&self, request: Request<InitFileRequest>) -> Result<Response<FileResponse>, Status> {
        let req = request.into_inner();
        ...
        let command = InitFileCommand::new(destination, owner_id, req.name, req.size);
    ...
    async fn upload_content(&self, request: Request<Streaming<FileChunk>>) -> Result<Response<()>, Status> {
        ...
        let service_handle = tokio::spawn(async move {
            app_state_clone.file_service.upload_stream(file_id, rx).await
        });
    ...
    async fn rename_file(&self, request: Request<RenameFileRequest>) -> Result<Response<FileResponse>, Status> {
        let req = request.into_inner();
        ...
        let file = self.app_state.file_service.update_file_name(command, file_id).await?;

        Ok(Response::new(map_file_to_proto(file)))
    }

    async fn undelete_file(&self, request: Request<UndeleteFileRequest>) -> Result<Response<FileResponse>, Status> {
        let req = request.into_inner();

        let file_id = map_entity_id(req.id)?;

        let file = self.app_state.file_service.update_deleted_file(file_id).await?;
    ...
    async fn delete_chosen_files(&self, request: Request<DeleteChosenFilesRequest>) -> Result<Response<()>, Status> {
        ...
        self.app_state.file_service.delete_chosen_files(&file_ids).await?;

        Ok(Response::new(()))
    }

    async fn delete_file(&self, request: Request<DeleteFileRequest>) -> Result<Response<()>, Status> {
        let req = request.into_inner();
